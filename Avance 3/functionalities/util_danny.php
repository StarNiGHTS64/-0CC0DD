...
function dispMision($idComp){
    ...
    function dispNinosNoEquipo(){
    
    $conn=connectDB();
    $sql="SELECT nombre, apellidoPaterno, apodo FROM nino WHERE idNino NOT IN (SELECT idNino FROM nino_equipo)";
