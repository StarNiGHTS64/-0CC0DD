<?php
	if (isset($_POST['key'])) {
		...
		if ($_POST['key'] == 'getRowData') {
			...
			$sql = $conn->query("SELECT nombrePersonaje, biografia, fuerza, mente, simpatia, magia, cultivos, tecnologia, carisma, img FROM personaje WHERE id='$rowID'");
		...
		if ($_POST['key'] == 'getExistingData') {
			...
			$sql = $conn->query("SELECT id, nombrePersonaje, img FROM personaje LIMIT $start, $limit");
			if ($sql->num_rows > 0) {
				$response = "";
        ...
        if ($_POST['key'] == 'updateRow') {
			$conn->query("UPDATE country SET nombrePersonaje='$nombrePersonaje', biografia='$biografia', fuerza='$fuerza', mente='$mente', simpatia='$simpatia', magia='$magia', cultivos='$cultivos', tecnologia='$tecnologia', carisma='$carisma', img='$img' WHERE id='$rowID'");
			exit('success');
		}
		...
		if ($_POST['key'] == 'addNew') {
			...
			if ($sql->num_rows > 0)
				...
				$conn->query("INSERT INTO personaje (nombrePersonaje, biografia, fuerza, mente, simpatia, magia, cultivos, tecnologia, carisma) 
							VALUES ('$nombrePersonaje', '$biografia', '$fuerza', '$mente', '$simpatia', '$magia', '$cultivos', '$tecnologia', '$carisma', '$img')");
